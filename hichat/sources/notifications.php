	...
	if(isset($_SESSION['username']) && isset($_SESSION['password']) || isset($_COOKIE['username']) && isset($_COOKIE['password'])) {	
		...
		if(empty($verify['username'])) {
			...
			if($_GET['filter'] == 'likes') {
				$x = $feed->checkNewNotifications($settings['nperpage'], 2, 1, null, null, null);
			} elseif($_GET['filter'] == 'comments') {
				$x = $feed->checkNewNotifications($settings['nperpage'], 2, null, 1, null, null);
			} elseif($_GET['filter'] == 'messages') {
				$x = $feed->checkNewNotifications($settings['nperpage'], 2, null, null, 1, null);
			} elseif($_GET['filter'] == 'chats') {
				$x = $feed->checkNewNotifications($settings['nperpage'], 2, null, null, null, 1);
			} else {
				$x = $feed->checkNewNotifications($settings['nperpage'], 2, 1, 1, 1, 1);
			}
			...
			$TMPL['editprofile'] = $feed->fetchProfileWidget($verify['username'], $verify['name'], $verify['image']);
