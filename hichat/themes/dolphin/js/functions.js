...
function showNotification(x) {
	if(x == 'close') {
		...
		$('#notifications_btn').removeClass('menu_hover');
		checkNewNotifications();
	} else {
		$('.notification-container').show();
		$('#notifications_btn').addClass('menu_hover');
...
function gallery(id, uid) {
	...
	// Put the content
	$('.image-content').html('<img src="uploads/media/'+id+'" class="ri" />').fadeIn(300);
	$('.gallery-footer-container').html('<div class="message-avatar">'+$('#avatar'+uid).html()+'</div><div class="message-top"><a onclick="gallery(\'close\')" title="Close Gallery"><div class="delete_btn"></div></a><a href="uploads/media/'+id+'" title="Download Image" target="_blank"><div class="download_btn"></div></a><div class="message-author">'+$('#author'+uid).html()+'</div><div class="message-time">'+$('#time'+uid).html()+'</div></div>');
	jQuery('div.timeago').timeago();
