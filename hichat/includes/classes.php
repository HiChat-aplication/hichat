...
class feed {
	...
	function getType($type, $value, $id) {
		...
		switch($type) {
			...
			case "visited":
				return '<div class="message-type-food"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/hiking.png" />'.sprintf($LNG['visited'], $value).'</div>
				<div class="message-divider"></div>';
				break;
			
			// If it's a camp action
			case "camp":
				return '<div class="message-type-food"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/camp.png" />'.sprintf($LNG['camp'], $value).'</div>
				<div class="message-divider"></div>';
				break;
	...
	function validateMessage($message, $image, $type, $value, $privacy) {
		...
		if($x == 1) {
			...
			$allowedType = array('map', 'camp', 'video', 'food', 'visited', 'movie', 'music');
